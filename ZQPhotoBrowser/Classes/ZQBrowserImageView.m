...
@implementation ZQBrowserImageView {
    __weak ZQWaitingView *_waitingView;
    BOOL _didCheckSize;
    CGFloat _totalScale;
}
...
- (void)layoutSubviews{
    ...
    if (self.bounds.size.width * (imageSize.height / imageSize.width) > self.bounds.size.height) {
        if (!_scroll) {
            ...
            scroll.backgroundColor = [ZQPhotoBrowserConfig shared].ZQPhotoBrowserBackgrounColor;
            _scroll = scroll;
...
- (void)setImageWithURL:(NSURL *)url placeholderImage:(UIImage *)placeholder{
    ZQWaitingView *waiting = [[ZQWaitingView alloc] init];
    waiting.bounds = CGRectMake(0, 0, 100, 100);
    waiting.mode = [ZQPhotoBrowserConfig shared].ZQWaitingViewProgressMode;
    ...
    [self sd_setImageWithURL:url placeholderImage:placeholder options:SDWebImageRetryFailed progress:^(NSInteger receivedSize, NSInteger expectedSize, NSURL * _Nullable targetURL) {
        ws.progress = (CGFloat)receivedSize / expectedSize;
        ...
        if (error) {
            ...
            ws.scrollImageView.image = image;
            [ws.scrollImageView setNeedsDisplay];
        }
    }];
}
...
- (void)prepareForImageViewScaling {
    if (!_zoomingScroolView) {
        ...
        _zoomingScroolView.backgroundColor = [ZQPhotoBrowserConfig shared].ZQPhotoBrowserBackgrounColor;
